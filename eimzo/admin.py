...
from eimzo.excel import download_as_excel_full, download_as_excel_short
...
@admin.action(description="Download as excel full")
def action_download_as_excel_full(modeladmin: admin.ModelAdmin, request: HttpRequest, queryset: QuerySet[Key]) -> HttpResponse:  # noqa: E501
    output = download_as_excel_full(queryset)
...
@admin.action(description="Download as excel short")
def action_download_as_excel_short(modeladmin: admin.ModelAdmin, request: HttpRequest, queryset: QuerySet[Key]) -> HttpResponse:  # noqa: E501
    output = download_as_excel_short(queryset)
    response = HttpResponse(
        output.read(),
        content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    response['Content-Disposition'] = 'attachment; filename=Kalitlar.xlsx'
    return response
...
class KeyAdmin(admin.ModelAdmin):
    ...
    actions = (action_download_as_excel_short, action_download_as_excel_full)
